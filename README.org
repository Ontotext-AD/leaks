...
*** Unicode problems
- Addresses and Officers are not in UTF-8 because they contain special characters like for example: C√¥te d'Ivoire, Cura√ßao, EL√?AS BAYTER MONTENEGRO, MARITZA GARCIA ALC√?NTARA etc. 

** Geonames and DBpedia Geography
...
- We extracted the data for all countires from [[http://ff-news.ontotext.com/sparql]]. It contains all statements from DBpedia and Geonames with sameAs statements between these datasets. For the purpose we used the next query:
#+BEGIN_EXAMPLE
...
- Aland Islands !√Öland Islands: the first is used for sorting, and the second is the page title
